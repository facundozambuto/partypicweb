	...
	try
	{
		...
		if(!empty($id_usuario))
		{
			if(!empty($search_phrase))
			{
				...
				$where  = "WHERE (ev.nombre_evento LIKE '".$search."%' OR sal.nombre_salon LIKE '".$search."%' OR ev.descripcion LIKE '".$search."%') AND sal.id_usuario = '".$id_usuario."' ";
			}
			else
			{
				$where  = "WHERE sal.id_usuario = '".$id_usuario."' ";
			}
		}
