<?php
	...
	try
	{
		...
		if(!empty($id_salon))
		{
			$where  = "WHERE sal.id_salon= '".$id_salon."' AND sal.id_usuario = '".$id_usuario."'";
			
		}
		else
		{
			$search = trim($search_phrase);
			$where  = "WHERE nombre_evento LIKE '".$search."%' OR nombre_salon LIKE '".$search."%' OR descripcion LIKE '".$search."%' ";
		}
